...
export default function AddExpenseModal({ isOpen, onClose }: AddExpenseModalProps) {
  ...
  const accountName = currentUser === 'unni' ? '언니 계정 🎀' : '동생 계정 🐻';
      ...
      <div className="bg-white border-t sm:border border-slate-200 rounded-t-3xl sm:rounded-3xl w-full max-w-md p-6 space-y-5 shadow-2xl animate-slide-up relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white font-black rounded-2xl shadow-lg shadow-emerald-500/25 transition-transform active:scale-95 text-base"
          >
            {isSubmitting ? '기록 중...' : '지출 완료하기 🚀'}
          </button>
        </form>
      </div>
    </div>
